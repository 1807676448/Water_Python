...
Water_Python 是一套面向水质参数预测的**神经网络训练与嵌入式部署工具链**。项目通过 BP 神经网络建立从补偿光谱输入到水质指标输出的非线性映射模型，配套串口数据采集工具，支持从数据采集→训练→导出→STM32 部署的完整闭环。

### 核心能力

- **串口数据采集**：基于 pyserial 的 GUI 工具，通过串口发送 HEX 触发指令，接收传感器 JSON 数据，用户补充实测值后自动追加到 Excel
...
| 项目 | 说明 |
|------|------|
| 输入 | `comp_254`（补偿后 254nm 吸光度）、`comp_550`（补偿后 550nm 吸光度） |
| 输出 | `COD_lab`（实验室实测 COD）、`UV254_lab`（实验室实测 UV254） |
| 网络结构 | 单隐层 BP：2 → 3 → 2（17 参数） |
| 隐层激活 | `tanh`（tansig） |
| 输出激活 | 线性（purelin） |
...
├── serial_collector.py             # 串口数据采集 GUI 工具
├── water_quality_ai.c              # STM32 推理引擎实现
├── water_quality_ai.h              # STM32 推理引擎接口声明
├── main.c                          # 历史示例代码
├── collected_data.xlsx             # 训练数据文件（由 serial_collector.py 生成）
...
| `pyserial` | 串口通信（serial_collector.py） | ≥3.5 |
...
# 2. 安装 Python 依赖
pip install numpy pandas torch scikit-learn joblib openpyxl pyserial
...
所有可调整参数集中在 [new.py](new.py) 顶部，以模块级常量形式定义：

| 参数名 | 默认值 | 说明 |
|--------|--------|------|
| `SEED` | `42` | 随机种子，影响数据划分、初始化与训练可复现性 |
| `DEFAULT_EXCEL_FILE` | `'collected_data.xlsx'` | 训练数据文件（由 serial_collector.py 生成） |
| `DEFAULT_TEST_SIZE` | `0.2` | 测试集占比（20%） |
| `DEFAULT_SCENARIO` | `'custom'` | 训练场景：`'custom'` / `'temperature_compensation'` / `'turbidity_prediction'` |
| `DEFAULT_INPUT_COLS` | `('comp_254', 'comp_550')` | 输入特征列名（需与 Excel 列名一致） |
| `DEFAULT_TARGET_COLS` | `('COD_lab', 'UV254_lab')` | 输出目标列名（需与 Excel 列名一致） |
| `DEFAULT_HIDDEN_SIZE` | `3` | 隐层神经元数量 |
| `DEFAULT_LEARNING_RATE` | `0.01` | Adam 学习率 |
| `DEFAULT_MAX_EPOCHS` | `10000` | 最大训练轮数 |
| `DEFAULT_TARGET_MSE` | `1e-4` | 早停目标误差（达到后提前终止） |
| `DEFAULT_TRAIN_ALGO` | `'adam'` | 优化算法：`'adam'` / `'gd'` / `'trainlm_like'` |
| `DEFAULT_WEIGHT_DECAY` | `5e-3` | L2 正则化系数 |
| `DEFAULT_EARLY_STOP_PATIENCE` | `800` | 验证损失不降多少轮后提前停止 |
| `DEFAULT_USE_PSO_INIT` | `True` | 是否启用 PSO 预初始化权重 |
| `DEFAULT_PSO_PARTICLES` | `30` | PSO 粒子数量 |
| `DEFAULT_PSO_ITERS` | `100` | PSO 最大迭代次数 |
...
训练数据文件（默认 `collected_data.xlsx`）由 `serial_collector.py` 自动生成，包含以下列：

| 列名 | 类型 | 说明 |
|------|------|------|
| `timestamp` | 字符串 | 采集时间戳 |
| `device_id` | 整数 | 设备 ID |
| `base_550` / `base_254` | 浮点 | 基线（暗电流）读数 |
| `comp_550` / `comp_254` | 浮点 | **补偿后读数（训练输入）** |
| `raw_550` / `raw_254` | 浮点 | 原始读数 |
| `Temp` | 浮点 | 温度（℃） |
| `COD_lab` | 浮点 | **实验室实测 COD（训练目标）** |
| `UV254_lab` | 浮点 | **实验室实测 UV254（训练目标）** |

训练时仅使用 `comp_254`、`comp_550` 作为输入，`COD_lab`、`UV254_lab` 作为输出。
时间戳和设备 ID 等非数值列在数据统计阶段自动跳过。

---

## 使用示例

### 工作流一：串口采集 + Python 训练闭环

```bash
# 步骤 0：启动串口采集工具，收集训练数据
python serial_collector.py
# 选择串口 → 打开 → 点击"开始测试" → 输入实测 COD/UV254 → 保存
# 数据自动追加到 collected_data.xlsx
...
python use_model.py
# 交互输入示例：
# 请输入输入值(空格分隔，输入 q 退出): -1.05 0.985
# 预测结果 -> COD_lab: 10.234567，UV254_lab: 0.270123
...
int main(void) {
    float input[INPUT_SIZE] = {-1.05f, 0.985f};
...
#### serial_collector.py — 串口采集模块

| 名称 | 类型 | 说明 |
|------|------|------|
| `SerialCollectorApp` | 类（`tk.Tk` 包装） | 串口采集 GUI 主窗口 |
| `_open_serial()` | 方法 | 打开串口（含 PermissionError 友好提示） |
| `_start_test()` | 方法 | 发送 HEX 触发指令 `AA AA BB BB` 并异步接收 JSON |
| `_read_until_json(timeout)` | 方法 | 串口读取直到完整 JSON（含 in_waiting 降级方案） |
| `_save_to_excel()` | 方法 | 追加传感器数据 + 用户实测值到 Excel |
...
接口定义位于 [water_quality_ai.h](water_quality_ai.h)，实现位于 [water_quality_ai.c](water_quality_ai.c)。
...
#### WaterQuality_Predict2

```c
void WaterQuality_Predict2(float in_0, float in_1, float* out_values);
```

**双输入快捷接口**，当模型输入维度为 2 时的简化调用形式。

| 参数 | 类型 | 说明 |
|------|------|------|
| `in_0`, `in_1` | `float` | 两个输入值（对应 comp_254、comp_550） |
| `out_values` | `float*` | 输出向量（调用者分配） |

#### WaterQuality_Predict

```c
void WaterQuality_Predict(float in_comp254, float in_comp550,
                          float* out_cod, float* out_uv254);
```

**命名参数接口**，以命名参数形式传递输入，以独立指针接收双输出。
...
```text
┌──────────────────┐     ┌──────────────┐     ┌──────────────────┐
│ serial_collector  │ ──▶ │ collected_data│ ──▶ │     new.py       │
│ (串口采集 GUI)    │     │   .xlsx       │     │   (训练+评估)     │
└──────────────────┘     └──────────────┘     └────────┬─────────┘
                                                       │
                          ┌────────────────────────────┼────────────────────────┐
...
**解决**：
1. 确保使用 `serial_collector.py` 生成的 `collected_data.xlsx`（列名自动匹配）
2. 如果使用自定义 Excel，确保存在 `comp_254`、`comp_550`、`COD_lab`、`UV254_lab` 列
...
**调整建议**：
- 减小 `DEFAULT_HIDDEN_SIZE`（如从 3 改为 2）
- 增加 `DEFAULT_TEST_SIZE`（如从 0.2 改为 0.3）
- 增大 `DEFAULT_WEIGHT_DECAY`（如从 5e-3 改为 1e-2）
...
### Q8：串口打开时报 PermissionError（拒绝访问）

**原因**：目标串口被其他程序占用。

**解决**：
- 关闭 SSCOM、串口调试助手等串口工具
- 关闭 STM32CubeProgrammer / ST-LINK Utility
- 断开 VS Code 的 Serial Monitor 面板
- 检查是否有其他 Python 进程占用该串口

### Q9：串口发送指令后 STM32 无响应

**原因**：串口配置不匹配或驱动问题。

**排查步骤**：
1. 确认波特率 115200、数据位 8、停止位 1、无校验
2. 确认 TX/RX 未交叉接反
3. 部分 STM32 虚拟串口驱动在 Windows 上 `in_waiting` 不可靠，软件已内置逐字节读取降级方案，超时后会输出 DEBUG HEX 信息
4. 关闭后重新打开串口尝试
...
- 输入：`comp_254`、`comp_550`
- 输出：`COD_lab`、`UV254_lab`
- 网络：单隐层 BP 神经网络，默认 `2 -> 3 -> 2` 结构
- 激活：隐层 `tanh`，输出层线性
- 归一化：`MinMaxScaler(-1, 1)`
- 训练策略：Adam / SGD / L-BFGS、PSO 初始化、L2 正则化、早停、学习率调度

### 输出目录统一管理

所有训练与导出结果统一写入 [outputs/](outputs/)：
...
- `training_log.txt`
- `model_data.h`
- `ui_param_memory.json`
- `ui_train_config.json`
- `ui_train_config_test.json`

### 关键脚本运行思路

`new.py` 负责读取训练配置、解析输入输出列、划分数据集、执行归一化与训练、输出评估指标，并将模型与配置保存到 `outputs/`。

`use_model.py` 从 `outputs/` 加载模型、归一化器与配置，接收输入向量后完成推理并输出反归一化结果。

`export_to_c.py` 提取权重、偏置与归一化参数，生成 `outputs/model_data.h`，供 STM32 工程直接引用。

`ui_app.py` 将训练、导出、模型加载和预测流程集成到图形界面中，适合日常交互使用。

`water_quality_ai.c/.h` 是 STM32 侧推理实现与接口声明，实际部署建议配合 `outputs/model_data.h` 一起使用。

`main.c` 属于历史示例代码，不建议作为当前版本的部署入口。

### 推荐流程

Python 端闭环建议按以下顺序执行：

1. 准备数据并运行 [serial_collector.py](serial_collector.py) 采集训练样本
2. 运行 [new.py](new.py) 完成训练
3. 运行 [use_model.py](use_model.py) 或 [ui_app.py](ui_app.py) 验证推理
4. 运行 [export_to_c.py](export_to_c.py) 导出 C 头文件

STM32 端闭环建议按以下顺序执行：

1. 训练完成后重新导出 `outputs/model_data.h`
2. 将 [water_quality_ai.c](water_quality_ai.c)、[water_quality_ai.h](water_quality_ai.h) 和 `outputs/model_data.h` 拷贝到 STM32 工程
3. 在工程中通过 `WaterQuality_Predict_Array` 或 `WaterQuality_Predict` 进行推理

### 当前数据与模型建议

当前数据量较小，模型更适合做趋势判断和原型验证，不建议直接用于生产决策。若想提升效果，优先考虑以下方向：

- 扩大样本量，尤其是高低值样本
- 放宽 550nm 和温度的变化范围
- 视任务拆分 COD 与 UV254 的独立模型
- 对小数据集先做线性回归或交叉验证作为基线

### 预测与部署注意事项

- 每次重新训练后都必须重新执行导出流程，保持 Python 端与 STM32 端参数同步
- `main.c` 不是当前导出链路的一部分，不要直接拿来替代 `water_quality_ai.c`
- 如果串口数据采集时提示占用或无响应，优先检查串口是否被其他工具占用，以及波特率是否为 115200
...
- STM32 推理引擎（推荐）：[water_quality_ai.c](water_quality_ai.c) / [water_quality_ai.h](water_quality_ai.h)
- 历史示例（不推荐用于当前版本）：[main.c](main.c)
